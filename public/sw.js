...
self.addEventListener('fetch', (event) => {
  ...
  if (request.mode === 'navigate') {
    event.respondWith(
      fetch(request)
        .then((response) => {
          // V37.0 — FIX: la risposta veniva messa in cache come shell
          // SENZA controllare lo stato. Un 500/502 di Vercel diventava
          // così la pagina servita offline, in modo permanente, finché
          // una navigazione non andava a buon fine. Si aggiorna la shell
          // solo su una risposta davvero valida.
