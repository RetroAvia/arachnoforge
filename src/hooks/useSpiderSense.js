...
export function useSpiderSense(materie, dayKey = null) {
  return useMemo(() => {
    // V37.0 — Le Materie con esame già verbalizzato escono da TUTTO lo
    // Spider-Sense. Prima venivano incluse: i loro nodi continuavano a
    // scadere e a chiedere ripassi per un esame già dato, gonfiando il
    // contatore dei "ripassi in sospeso" con lavoro che non serve più.
    // Il reducer chiude già i nodi alla convalida (vedi UPDATE_MATERIA),
    // ma questo filtro copre anche i profili salvati prima della V37 e
    // qualunque dato arrivato da un import.
    const attive = (Array.isArray(materie) ? materie : []).filter((m) => m && !m.examPassed);
    const allTrackedReviews = attive.flatMap((m) =>
      m.sfide
    ...
    const memoryRadar = {
      global: buildRadar(attive.flatMap((m) => m.sfide)),
      byMateria: attive.map((m) => ({ materiaId: m.id, materiaNome: m.nome, ...buildRadar(m.sfide) }))
    };

    return { allTrackedReviews, upcomingReviews, goblinMaterie, memoryRadar };
    // `dayKey`: i ripassi "scaduti" dipendono da oggi, non solo dalle
    // materie — senza, a cavallo della mezzanotte restavano quelli di ieri.
