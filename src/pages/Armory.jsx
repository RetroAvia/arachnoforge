...
import { CARD, H1, H2, BTN_SECONDARY, BTN_AMBER, BTN_GHOST, INPUT, BADGE } from '../utils/designSystem.js';
...
export default function Armory() {
  ...
  // chiusura: prima chiudere il drawer, o cambiare pagina, entro 400 ms
  // dall'ultimo tasto perdeva le ultime parole scritte.
  ...
  const trophiesByTier = useMemo(() => {
    // V39 — i gruppi nascono da TIER_ORDER: il quarto tier (Vibranium)
    // mancava dall'elenco scritto a mano e i suoi trofei non comparivano mai.
  ...
  return (
    <div className="space-y-9">
      ...
      <div
        className="flex items-center gap-2 border-b border-secondary/15 pb-0 overflow-x-auto af-scroll"
        role="tablist"
        aria-label="Sezioni Suit Lab"
        onKeyDown={(e) => {
          // V39 — pattern WAI-ARIA Tabs: con tabIndex -1 sulle schede
          // inattive servono le frecce, altrimenti da tastiera la seconda
          // scheda era irraggiungibile.
            ...
            {blueprintNodes.map((entry) => (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <p className="text-base font-medium leading-snug line-clamp-2 break-words min-w-0 text-slate-100">{entry.sfida.nome}</p>
              ...
              return (
                <div key={r.id} className={`${CARD} flex items-center justify-between gap-3`}>
                  <div className="relative min-w-0 flex-1">
                    <p className="text-base font-medium leading-snug text-slate-100 line-clamp-2 break-words">{r.nome}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
            {state.inventory.map((item) => (
              <div key={item.id} className={`${CARD} flex items-center justify-between gap-3`}>
                <div className="relative min-w-0 flex-1">
                  <p className="text-base font-medium leading-snug text-slate-100 line-clamp-2 break-words">{item.nome}</p>
          ...
          {TIER_ORDER.map((tier) => {
            ...
            return (
              <div key={tier}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5">
                  {trophies.map((t) => (
                    <div
                      key={t.id}
