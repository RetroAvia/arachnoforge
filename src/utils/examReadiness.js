...
export function computeExamReadiness(materia, radar = null, calibration = null) {
  ...
  // --- Copertura, pesata per ore stimate ---------------------------------
  const totalHours = sfide.reduce((sum, s) => sum + Math.max(0.5, Number(s.oreStimate) || 0), 0);
  const doneHours = sfide
    .filter((s) => s.status === PERSISTED_STATUS.COMPLETED)
    .reduce((sum, s) => sum + Math.max(0.5, Number(s.oreStimate) || 0), 0);
  const hasNodes = sfide.length > 0;
  const coverage = hasNodes && totalHours > 0 ? doneHours / totalHours : 0;
  ...
  const known = [hasNodes, hasStability, !!materia?.examDate, hasFriction].filter(Boolean).length;
  ...
  const gaps = [
    ...
    { key: 'feasibility', deficit: (1 - feasibility) * WEIGHTS.feasibility, enabled: !!materia?.examDate },
  ...
  return {
    ...
    known: { hasNodes, hasStability, hasExamDate: !!materia?.examDate, hasFriction },
