...
/**
 * V39.0 — Stessa card, SENZA colore del bordo: per le card che il colore
 * del bordo lo decidono da sole (stato di un nodo, esito di una
 * simulazione). Aggiungere `border-emerald-400/40` a CARD non bastava:
 * due classi di colore del bordo con la stessa specificità, e vinceva
 * quella che Tailwind genera per ultima nel foglio — di solito
 * `border-secondary/20` di CARD, cioè il colore "giusto" non si vedeva.
 */
export const CARD_BARE =
  'bg-surface/70 backdrop-blur-lg border rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] p-4 sm:p-6 relative overflow-hidden';
...
/**
 * V38.0 — Variante compatta dello stesso input, per le griglie di campi
 * numerici stretti (le fonti di un nodo, i due campi del Debriefing).
 * Esiste come token e non come stringa locale in due file perché era
 * già stata copiata a mano due volte con due valori di placeholder
 * diversi — e in uno dei due il placeholder porta informazione reale
 * (le pagine attese al ritmo corrente), quindi il contrasto non era
 * negoziabile. Stessa grammatica di INPUT: scala ridotta, mai un altro
 * linguaggio visivo.
 */
export const INPUT_SM =
  'w-full bg-surface/80 border border-secondary/30 rounded-lg px-2.5 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors appearance-none';
