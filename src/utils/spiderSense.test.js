...
describe("capIntervalToExam — nessun ripasso schedulato dopo l'esame", () => {
  ...
  // V39.0 — il test precedente fissava come corretto il comportamento
  // sbagliato: con l'esame domani il ripasso veniva schedulato DOPO.
  test("a esame domani il ripasso cade al più tardi il giorno dell'esame, mai dopo", () => {
    assert.equal(capIntervalToExam(5, isoInDays(1)), 1);
    assert.equal(capIntervalToExam(69, isoInDays(1)), 1);
  });

  test("a esame oggi o già passato l'intervallo segue la curva normale", () => {
    assert.equal(capIntervalToExam(5, isoInDays(0)), 5);
