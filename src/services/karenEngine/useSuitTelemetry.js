...
export function useSuitTelemetry() {
  ...
  useEffect(() => {
    fetchAll();
    // V37.0 — Supabase invoca il listener SUBITO con `INITIAL_SESSION`,
    // quindi il primo evento duplicava esattamente la fetch appena
    // lanciata: sei query invece di tre, ad ogni boot. Si ignora solo
    // quel primo evento — ogni transizione reale (login, logout, refresh
    // del token) continua a far ripartire il fetch come prima.
  ...
  const generateNodeQuiz = useCallback(async (materiaId, sfidaId) => {
    ...
    // (MAX_QUIZ_GENERATIONS_PER_DAY). Senza, il server ricadrebbe sulla
    // data UTC e il tetto si azzererebbe a un'ora sbagliata per chi vive
    // a est di Greenwich — lo stesso Timezone Trap già risolto per il
    // briefing nella v2.
    const invokeOnce = () =>
      supabase.functions.invoke('karen-oracle', {
        body: { mode: 'quiz', materiaId, sfidaId, date: currentDateRef.current || todayDateOnlyKey() }
      });
  ...
  // V37.0 — PRESTAZIONI: era un letterale nuovo ad ogni render, e questo
  // rendeva del tutto inutile il `useMemo` di KarenBrainContext (che
  // memoizza proprio su questo oggetto): ogni render del Provider
  // biometrico propagava un `value` nuovo a tutti i consumatori, MissionControl
  // e ArachnoForgeContext inclusi. Ora l'identità cambia solo quando
