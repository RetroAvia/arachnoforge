...
export default function NexusGate() {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      // V37.0 — La soglia sale da 6 a 10 caratteri, ma SOLO in
      // registrazione: alzarla anche al login bloccherebbe fuori un
      // account già creato con una password più corta, che è l'ultima
      // cosa che deve succedere a chi ha i propri dati dentro.
      // Il minimo va alzato anche lato server (Supabase -> Auth ->
      // Password requirements): questa è solo la prima linea.
      const minLength = mode === MODES.SIGNUP ? 10 : 6;
      if (password.length < minLength) {
        setError(`Karen: la password deve contenere almeno ${minLength} caratteri per superare i firewall Stark.`);
        audio.playAccessDenied();
        return;
      }
        ...
        <div className="relative bg-surface/60 backdrop-blur-2xl border border-secondary/25 rounded-3xl shadow-[0_20px_70px_rgba(0,0,0,0.6)] p-7 sm:p-8 overflow-hidden af-nexus-card-in">
          ...
          <div className="relative grid grid-cols-2 bg-surface/70 border border-white/10 rounded-xl p-1 mb-6">
            <div
              ...
              style={{ transform: mode === MODES.SIGNUP ? 'translateX(100%)' : 'translateX(0)' }}
            />
            <button
              type="button"
