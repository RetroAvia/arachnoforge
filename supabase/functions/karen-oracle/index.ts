...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      return jsonResponse({ error: 'Authorization header mancante.' }, 401);
    }
    ...
    if (userError || !user) {
      ...
      return jsonResponse({ error: 'Sessione non valida o scaduta.', code: 'INVALID_SESSION' }, 401);
    }
    ...
    if (existing && !force) {
      return jsonResponse({ briefing: existing, readiness_band: readinessBand(existing.readiness_score), cached: true });
    }
    ...
    if (saveError) {
      ...
      return jsonResponse({ error: 'Briefing generato ma non salvato.', details: saveError.message }, 500);
    }

    return jsonResponse({ briefing: saved, readiness_band: band, cached: false });
  } catch (err) {
    console.error('karen-oracle: errore inatteso', err);
    return jsonResponse({ error: 'Errore interno di K.A.R.E.N.' }, 500);
  }
});
